...
public class Widget_Page {
	...
	public void verify_widgetTab() throws Exception {
		try {
			...
			widgetPage.clickOnWidgetTab(driver);
			
			// Verify links present on Widget Tab
			LoggerInstance.logger.info("verifying links present on Widget tab");
			boolean isVerifyLinksPresent = widgetPage.verifyLinksPresent(driver);
...
//			boolean isVerifysectionby = widgetPage.verifysectionby(driver);
...
//			widgetPage.clickOnWidgetTab(driver);
			...
			boolean isVerifyLatestImpression = widgetPage.verifyLatestImpression(driver);
